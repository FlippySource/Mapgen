...
public class Mapgen
{
	...
	public int [] [] recursiveBranchingFill(int [][] board, int boardx,  int boardy, int roomcount, int pixelfills, Mapgen mapgen, Random random)
	{ //fill 1 random, recurse once for each created pixel with x chance of adjacent
	...
	public ArrayList recurseFill(int [][] bd, int boardx, int boardy, Random rand, int px, int py, int pixelfills, int depth)
	{//something is exiting its recursion early and not handing back up some information
		int [][] board = bd;
		if (depth > 1200)
		{
	...
	Public int [][] bridgeBuilder(int [][] board)
	{
		...
		for(int y = 0; y < board [0].length; y++)
		{
			for(int x = 0; x < board.length; x++)
			{
				...
				{
					segEnd = x;
					//find if there is 1 within 1/12 of the board below to bridge too (at least x len?)
				}
